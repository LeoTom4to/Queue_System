...
class MMc:
    ...
    def __init__(self,run_rounds = 10,total_service_time=0,server_time_mean = 2,customers=10000,mu_rate=0.5,desks=6,lambda_rates=5):
    ...
    def plots(self,x,y,title,y_label):
            ...
            plt.ylabel(y_label)
            plt.grid(True)
            if not os.path.exists('MMC_plots'):
                os.makedirs('MMC_plots')
            plt.savefig(f'MMC_plots/{title}.png')
...
if __name__=='__main__':
    lambda_rates= [1,1.25,1.5,1.75,2,2.25,2.5,2.75,3,3.25,3.5,3.75,4,4.25,4.5,4.75,5.5,6,6.5,7,7.5,8,8.5,9]
    num_repetitions=20
    mmc=MMc()
